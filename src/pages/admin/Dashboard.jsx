...
export default function AdminDashboard() {
  ...
  const fetchMasterSheetColumnA = async () => {
    try {
      ...
      const response = await fetch(`https://docs.google.com/spreadsheets/d/1URUsMsKAE74rTANPwgUtl-8N7wITiW_kVOqDeCbdi7k/gviz/tq?tqx=out:json&sheet=MASTER`)
      ...
      // Count active staff (column C), also skipping header row
  ...
  const fetchDepartmentData = async (department) => {
    ...
    try {
      ...
      const response = await fetch(`https://docs.google.com/spreadsheets/d/1URUsMsKAE74rTANPwgUtl-8N7wITiW_kVOqDeCbdi7k/gviz/tq?tqx=out:json&sheet=${sheetName}`);
      ...
      // FIXED: Get tomorrow's date for comparison
      const tomorrow = new Date();
      tomorrow.setDate(tomorrow.getDate() + 1);
      tomorrow.setHours(0, 0, 0, 0);
      ...
      const processedRows = data.table.rows.map((row, rowIndex) => {
        ...
        // Debug: Log date validation for first few rows
        if (rowIndex <= 5) {
        ...
        if (shouldCountInStats) {
          ...
          if (status === 'completed') {
            ...
            // COUNTING LOGIC:
            // - Pending Tasks: ALL incomplete tasks (including overdue + today's tasks)
            // - Overdue Tasks: ONLY past dates (excluding today's date)
  ...
  useEffect(() => {
    if (dashboardType === "delegation") {
      // For delegation mode, always fetch delegation data
