...
export default function AssignTask() {
  ...
  const [departmentDoerMapping, setDepartmentDoerMapping] = useState({})
  ...
  const handleChange = (e) => {
    ...
    // Filter doers when department is selected
    if (name === "department") {
      if (value && departmentDoerMapping[value]) {
        setFilteredDoerOptions(departmentDoerMapping[value].sort())
      } else {
        setFilteredDoerOptions(doerOptions) // Show all doers if no department selected
      }

      // Reset doer selection if the current doer is not in the filtered list
      if (
        formData.doer &&
        value &&
        departmentDoerMapping[value] &&
        !departmentDoerMapping[value].includes(formData.doer)
      ) {
  ...
  const fetchMasterSheetOptions = async () => {
    try {
      const masterSheetId = "1URUsMsKAE74rTANPwgUtl-8N7wITiW_kVOqDeCbdi7k"
      ...
      // Extract options from columns A, B, C, and F
      const departments = []
      const givenBy = []
      const doers = []
      const deptDoerMap = {}
      ...
      data.table.rows.slice(1).forEach((row) => {
        ...
        // Column F - Department for each doer (for filtering)
        if (row.c && row.c[5] && row.c[5].v && row.c && row.c[2] && row.c[2].v) {
          const department = row.c[5].v.toString().trim()
          const doerName = row.c[2].v.toString().trim()

          if (department !== "" && doerName !== "") {
            if (!deptDoerMap[department]) {
              deptDoerMap[department] = []
            }
            if (!deptDoerMap[department].includes(doerName)) {
              deptDoerMap[department].push(doerName)
            }
          }
        }
      })
      ...
      setDepartmentDoerMapping(deptDoerMap)
      setFilteredDoerOptions([...new Set(doers)].sort()) // Initially show all doers

      console.log("Master sheet options loaded successfully", {
        departments: [...new Set(departments)],
        givenBy: [...new Set(givenBy)],
        doers: [...new Set(doers)],
        departmentDoerMapping: deptDoerMap,
      })
    } catch (error) {
      ...
      setFilteredDoerOptions(["Doer 1", "Doer 2"])
    }
  }
  ...
  const getLastTaskId = async (sheetName) => {
    try {
      const url = `https://docs.google.com/spreadsheets/d/1URUsMsKAE74rTANPwgUtl-8N7wITiW_kVOqDeCbdi7k/gviz/tq?tqx=out:json&sheet=${encodeURIComponent(
        sheetName,
      )}`
  ...
  const fetchWorkingDays = async () => {
    try {
      const sheetId = "1URUsMsKAE74rTANPwgUtl-8N7wITiW_kVOqDeCbdi7k"
        ...
        if (row.c && row.c[0] && row.c[0].v) {
          ...
          if (typeof dateValue === "string" && dateValue.startsWith("Date(")) {
            ...
            if (match) {
              ...
              const month = Number.parseInt(match[2], 10) // 0-indexed in Google's format
              const dateDay = Number.parseInt(match[3], 10)

              dateValue = `${dateDay.toString().padStart(2, "0")}/${(month + 1).toString().padStart(2, "0")}/${year}`
            }
  ...
  const findEndOfWeekDate = (date, weekNumber, workingDays) => {
    ...
    if (weekNumber === -1) {
      ...
      return (
        weekGroups[weekGroups.length - 1]?.[weekGroups[weekGroups.length - 1].length - 1] ||
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await fetch(
        "https://script.google.com/macros/s/AKfycby15yq3wOnFuTtHmfaEiVlnge042lvNeGt9ytefX6Lck7aKGKEPWOyQ21pcMiv5TmEm/exec",
        {
          method: "POST",
          body: formPayload,
          mode: "no-cors",
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="space-y-2">
                ...
                {formData.department && filteredDoerOptions.length === 0 && (
                  <p className="text-sm text-red-600">No doers found for the selected department.</p>
                )}
              </div>
                          ...
                          <div className="max-h-60 overflow-y-auto space-y-2">
                            {generatedTasks.slice(0, 20).map((task, index) => (
                              <div key={index} className="text-sm p-2 border rounded-md border-purple-200 bg-purple-50">
                                <div className="font-medium text-purple-700">{task.description}</div>
