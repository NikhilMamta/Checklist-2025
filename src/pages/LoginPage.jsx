  ...
  useEffect(() => {
    const fetchMasterData = async () => {
      ...
      try {
        ...
        const SPREADSHEET_ID = "1URUsMsKAE74rTANPwgUtl-8N7wITiW_kVOqDeCbdi7k";
        ...
        if (data.table && data.table.rows) {
          ...
          for (let i = 1; i < data.table.rows.length; i++) {
            ...
            const department = row.c[5] && String(row.c[5].v || "").trim();

            const mainAdminDoersName = row.c[8] && String(row.c[8].v || "").trim();
            const mainAdminGivenBy = row.c[1] && String(row.c[1].v || "").trim();

            // console.log("mainAdminDoersName:", mainAdminDoersName);

            if (department) {
              userDepartments[username] = department;
            }

            if(mainAdminDoersName){
              userDoersName[username] = mainAdminDoersName;
            }

            if(mainAdminGivenBy){
              userGivenBy[username] = mainAdminGivenBy;
            }
        ...
        setMasterData({ userCredentials, userRoles, userDepartments, userDoersName, userGivenBy });
    ...
    try {
      ...
      if (trimmedUsername in masterData.userCredentials) {
        ...
        if (correctPassword === trimmedPassword) {
          ...
          if (isAdmin || userRole === "main admin") {
          ...
          // Store the department from row[5] (column F)
          const userDepartment =
            masterData.userDepartments?.[trimmedUsername] || "";
          
          const userDoersName =
            masterData.userDoersName?.[trimmedUsername] || "";
          
          const userGivenBy =
            masterData.userGivenBy?.[trimmedUsername] || "";

          if (userDoersName) {
            sessionStorage.setItem("mainAdminDoersName", userDoersName);
          }

          if (userGivenBy) {
            sessionStorage.setItem("mainAdminGivenBy", userGivenBy);
          }


          if (userDepartment) {
            sessionStorage.setItem("departmentAdminMain", userDepartment);
          }
