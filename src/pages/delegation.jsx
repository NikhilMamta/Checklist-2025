...
const CONFIG = {
  ...
  DRIVE_FOLDER_ID: "1h_QZhuWxnCOBBCpyPKdX8Yx6hAakXXHe",
...
function DelegationDataPage() {
    ...
    const fetchDropdownData = async () => {
      try {
        const response = await fetch(
          `${CONFIG.APPS_SCRIPT_URL}?sheet=Whatsapp&action=fetch`
        );
    ...
    return historyData
      .filter((item) => {
        ...
        const userMatch =
          ...
          (userRole === "main admin" &&
            item["col10"] &&
            item["col10"].toLowerCase() ===
              sessionStorage.getItem("departmentAdminMain")?.toLowerCase()) ||
          (item["col7"] &&
            item["col7"].toLowerCase() === username.toLowerCase());
    ...
    try {
          ...
          if (historyData && historyData.table && historyData.table.rows) {
            processedHistoryData = historyData.table.rows
              .map((row, rowIndex) => {
                ...
                rowData["col10"] = rowValues[10]
                  ? parseGoogleSheetsDate(String(rowValues[10]))
                  : ""; // Column K - Task End Date
      ...
      rows.forEach((row, rowIndex) => {
        ...
        if (currentUserRole === "admin") {
          ...
          isUserMatch = true;
        } else if (currentUserRole === "main admin") {
          // Main admin sees only their department tasks
          const mainAdminDepartment =
            sessionStorage.getItem("departmentAdminMain") || "";
          isUserMatch =
            departmentName.toLowerCase() === mainAdminDepartment.toLowerCase();
        } else {
  ...
  // filteredHistoryData = userRole === "main admin" ? filteredHistoryData.filter(item=> item.col10 === sessionStorage.getItem("departmentAdminMain")) : filteredHistoryData;
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredHistoryData.length > 0 ? (
                      filteredHistoryData.map((history) => (
                        <tr
                          key={history._id}
                          className="hover:bg-blue-100 bg-blue-200"
                        >
                          ...
                          <td className="px-6 py-4 whitespace-nowrap">
                            {isSelected && editMode[account._id] ? (
                              ...
                              <div
                                ...
                                onClick={() =>
                                  isSelected && toggleEditMode(account._id)
                                }
                              >
                                {account["col2"] || "—"}
                                {isSelected && (
